...
styles.add(ParagraphStyle(name="TitleS05", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=17, alignment=TA_CENTER, spaceAfter=10))
styles.add(ParagraphStyle(name="BodyS05", parent=styles["BodyText"], fontSize=9.5, leading=13, spaceAfter=7))
styles.add(ParagraphStyle(name="HeadS05", parent=styles["Heading2"], fontSize=11.5, spaceBefore=12, spaceAfter=6))
styles.add(ParagraphStyle(name="CodeS05", fontName="Courier", fontSize=7.4, leading=9, backColor=colors.HexColor("#f2f4f6"), borderPadding=7, spaceAfter=8))
story = []
...
p("<b>Fuente de evidencia:</b> ejecución automatizada de los mismos endpoints de la colección de Postman; "
  "las respuestas completas y los estados de Compose quedaron en <font name='Courier'>resultados.json</font>. "
...
p("Cloné la etiqueta v05.1 del repositorio oficial. Al construir, el archivo <font name='Courier'>mvnw</font> tenía "
...
p("El endpoint <font name='Courier'>GET /actuator/health</font> devolvió HTTP 200 y estado UP.")
...
p("Envié dos veces el mismo SPEI de 50 con <font name='Courier'>Idempotency-Key: veraza-amy-s05-001</font>. "
...
p("Detuve app con <font name='Courier'>docker compose stop app</font>. Tras 15 segundos no volvió sola y "
  "la solicitud HTTP falló. La base siguió healthy. Al iniciar app otra vez, la cuenta conservó saldo 750. "
  "También sobrevivió a <font name='Courier'>docker compose down</font> y "
  "<font name='Courier'>docker compose up -d</font>. Después de <font name='Courier'>docker compose down -v</font> "
